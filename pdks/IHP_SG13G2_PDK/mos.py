...
class MOSGenerator(DefaultCanvas):
    ...
    def __init__(self, pdk, height, fin, gate, gateDummy, shared_diff, stack, bodyswitch, **kwargs):
        ...
        assert (self.finsPerUnitCell*self.pdk['Active']['Pitch'])%self.pdk['M2']['Pitch']==0
        self.m2PerUnitCell = (self.finsPerUnitCell*self.pdk['Active']['Pitch'])//self.pdk['M2']['Pitch']
        ...
        assert fin >= 1, "transistor width factor must be >= 1"
        assert gateDummy > 0
        unitCellLength = self.gatesPerUnitCell* self.pdk['Poly']['Pitch']
        activeOffset = self.unitCellHeight//2 -self.pdk['Active']['Pitch']//2
        ...
        offset_active_body = (self.lFin//2)*self.pdk['Active']['Pitch']+self.unitCellHeight-self.pdk['Active']['Pitch']//2
    ...
    def _addBodyContact(self, x, y, x_cells, yloc=None, name='M1'):
        ...
        body_v0_track = (self.lFin*self.pdk['Active']['Pitch'])//(2*self.pdk['M2']['Pitch'])
    ...
    def _addMOSArray( self, x_cells, y_cells, pattern, vt_type, connections, minvias = 1, **parameters):
        ...
        # Default centroid bounds for pattern==3 (CurrentMirror / ratio_devices).
        # The size-aware block below overrides these when len(parameters)>2 and
        # devices differ in NFIN*NF*M. For uniform 1:1 mirrors (e.g. matched
        # diode+mirror pair with equal sizing) the override does not fire,
        # leaving x_left/x_right undefined; without this init the pattern==3
        # branch in the per-cell loop raises UnboundLocalError. Symmetric
        # B|A|B split:
        #   x_cells=4 -> B A A B    x_cells=8 -> B B A A A A B B
        x_left = x_cells // 4
